...
- **Admin-Check:** Rollenbasiert über `public.user_roles` (Kompatibilitäts-Check via RPC `public.is_admin(uuid)`).
...
2. **Eingeloggt, nicht Admin:** User ohne Rolle `admin`/`owner` in `public.user_roles` → 403, Body z. B. `{ "error": "Keine Berechtigung." }`.
3. **Admin:** User mit Rolle `admin` oder `owner` in `public.user_roles` → 200/201 wie erwartet.
